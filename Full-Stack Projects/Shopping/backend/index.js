// initialization of dependency and module
const port = 4000;
...
mongoose.connect("mongodb+srv://kishortam:shoppingapp@cluster0.6wea3.mongodb.net/Shopping");
...
app.post('/signup', async(req, res)=>{
    ...
    const token  = jwt.sign(data, 'secret_shopping');
...
const fetchUser = async(req, res, next)=>{
    ...
    if(!token){
        ...
        try {
            const data = jwt.verify(token, 'secret_shopping');
            req.user = data.user;
            next();
        } catch (error) {
            res.status(401).send({errors:"Please authenticate using valid token"})
